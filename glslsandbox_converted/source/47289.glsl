...
vec4 aurora(vec3 ro, vec3 rd)
{
 ...
 //This controls the speed!!!
        float rzt = triNoise2d(p, .0936);
        vec4 col2 = vec4(0,0,0, rzt);
        col2.rgb = (sin(1.-vec3(2.15,-.5, 1.2)+i*time*0.43)*0.5+0.5)*rzt;
        avgCol =  mix(avgCol, col2, .5);
        col += avgCol*exp2(-i*0.065 - 2.5)*smoothstep(0.4,5., i);
 ...
 for (float i=0.;i<4.;i++)
    {
        ...
        float c2 = 1.-smoothstep(0.,.16,length(q));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
 ...
 mo.x *= iResolution.x/iResolution.y;
    ...
    vec3 col = vec3(0.);
    vec3 brd = rd;
    float fade = smoothstep(0.4,0.01,abs(brd.y))*0.1+0.9;
