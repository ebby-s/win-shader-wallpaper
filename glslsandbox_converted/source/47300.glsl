...
void main( void ) 
{
 ...
 float a = sin(position.y * 1.6 - time * 0.1) / 1.0;
 float b = cos(position.y * 1.4 - time * 0.2) / 1.0;
 float c = sin(position.x * 1.2 - time * 0.2 + 3.14) / 2.0;
 float d = cos(position.y * 2.6 - time * 0.5 + 3.14) / 2.0;
 
 float e = 0.61 / abs(position.x + a);
 float f = 0.41 / abs(position.x + b);
 float g = 0.51 / abs(position.y + c);
 float h = 0.51 / abs(position.x + d);
 
 vec3 color = rColor * e * gColor * f + bColor * f * g * h * 0.08;
