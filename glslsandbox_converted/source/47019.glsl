...
vec4 voronoi( in vec2 x, float mode )
{
 ...
 float m2 = 8.0;
  ...
  vec2 d0 = vec2( sqrt(dot(r,r)), 1.0 );
        // manhattam  
  vec2 d1 = vec2( 3.71*(abs(r.y) + abs(r.r)), 9.0 );
        // triangular  
  vec2 d2 = vec2( max(abs(r.x)*0.866025+r.y*0.5,-r.y), 
            step(0.0,0.5*abs(r.x)+0.866025*r.y)*(1.0+step(5.0,r.x)) );

  vec2 d = d0; 
  if( mode<4.0 ) d=mix( d2, d0, fract(mode) );
  if( mode<1.0 ) d=mix( d1, d2, fract(mode) );
  if( mode<4.0 ) d=mix( d0, d1, fract(mode) );
...
float mode = mod(time/5.0,3.0);
 ...
 col *= clamp( 5.5 + (1.0-c.z/2.0)*0.5, 0.0, 1.0 );
