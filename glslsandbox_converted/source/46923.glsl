...
void main( void ) {
 ...
 vec2 position = (( gl_FragCoord.xy / resolution.xy )-vec2(0.5))*resolution.xy/resolution.y*4.0;
 ...
 if(length(o)<=1.0){
  if(!close){
   ...
   for(int i=0;i<mx;i++){
    ...
    g=g*2.0;
     if(length(pt)<1.0/resolution.y){
      close=true;
      color=hsv2rgb(vec3(startM/2.0,1.0,1.0));
      break;
     }
