...
float trap(in vec3 p)
{
 ...
 return abs(length(p.xz)-0.2);
 return min(length(p.xz), min(length(p.yz), length(p.xy)));
...
float map(in vec3 p)
{
    float time2 = time+60.0*sin(3.);
 float cutout = dot(abs(p.yz),vec2(1.3*fract(time*.0+0.5)))-0.035;
  float road = max(abs(p.y-0.025), abs(p.z)-0.035);
 
 vec3 z = abs(1.0-mod(p,1.0));
 z.yz = rotate(z.yz, time2*0.0);
