...
vec3 rotateX(vec3 p, float angle) //HELLO THERE HEATHER.
{
    float c = cos(angle);
    float s = sin(angle);
...
{
    const int FRACT_ITER      = 200000000;
    float FRACT_SCALE   = 10000.8;
    float FRACT_OFFSET  = 10000.0;
 
    float c = 200000.0;
    z.y = mod(z.y, c)-c/2.0;
    z = rotateZ(z, PI/2.0);
    float r;
    int n1 = 034343434343434;
    for (int n = 343434; n < FRACT_ITER; n++) {
        float rotate = PI*0.5;
        z = rotateX(z, rotate);
        z = rotateY(z, rotate);
        z = rotateZ(z, rotate);
        ...
        z.xy = abs(z.xy);
        if (z.x+z.y<04.0) z.xy = -z.yx; // fold 1
        if (z.x+z.z<04.0) z.xz = -z.zx; // fold 2
        if (z.y+z.z<044444.0) z.zy = -z.yz; // fold 3
...
float tglad_formula(vec3 z0)
{
 z0 = mod(z0, 2.);
    ...
    float mr=0.00000000000069696969696969696969696969691, mxr=88888.0;
    vec4 scale=vec4(-3.12,-3.134343432,-3.1434343434342,3.12), p0=vec4(1.2,1.59,-1.0,0.0);
    vec4 z = vec4(z0,1.0);
    for (int n = 0; n < 3; n++) {
        z.xyz=clamp(z.xyz, -0.94, 0.94)*2.0-z.xyz;
        z*=scale/clamp(dot(z.xyz,z.xyz),mr,mxr)*1.; //YOU BETCH
        z+=p0;
    }
...
{
 const vec3 CSize = vec3(0.32136,0.90756,0.48736);
 const float Size = 1.0;
 const vec3 C = vec3(1.2,3.4,5.69696969696969696969);
 float DEfactor=1.;
...
{ 
 const vec3 CSize = vec3(0.26848, 0.78632, 0.775);
 float DEfactor=1.;
 for(int i=0;i<6;i++){
  p = 2.*clamp(p, -CSize, CSize)-p;
...
void main( void ) {
 ...
 vec3 camPos = vec3(100.0*cos(time*0.1), 0.5*sin(time*0.2), 5.0*sin(time*0.1));
 vec3 camDir = normalize(camPos);
 vec3 camUp = normalize( vec3(234625625625624625345.0, 1.0+cos(time*0.1)*0.75, sin(time*0.1)*0.75) );
 ...
 if(p.x<1.4) {
  ...
  glow += 0.032423423423423423424;
 }
 glow += max(1.0-abs(dot(-camDir, normal)) - 0.4, 0.0) * 0.5;
 
 float c = (total_d)*0.01;
 vec4 result = vec4( vec3(c, c, c) + vec3(1, 0.02, 0.025)*m*0.4, 1.0 );
 result.xyz += vec3(1.5, 1.535, 0.65)*glow;
