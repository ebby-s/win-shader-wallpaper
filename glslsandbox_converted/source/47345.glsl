...
vec2 position(float z) {
 return vec2(
  0.0 + sin(z * 0.1) * 1.0 + sin(cos(z * 0.031) * 4.0) * 1.0 + sin(sin(z * 0.0091) * 3.0) * 0.0,
  0.0 + cos(z * 0.1) * 1.0 + cos(cos(z * 0.031) * 4.0) * 1.0 + cos(sin(z * 0.0091) * 3.0) * 0.0
 ) * 1.0;
}

void main(void){
 vec2 p = (gl_FragCoord.xy * 2.0 - resolution) / min(resolution.x, resolution.y);
 float camZ = 15.0 * time;
 vec2 cam = position(camZ);

 float dt = 0.5;
 float camZ2 = 15.0 * (time + dt);
